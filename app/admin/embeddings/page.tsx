...
export default function AdminEmbeddingsPage() {
  ...
  async function resolveAlert(id: string) {
    try {
      const res = await fetch('/api/admin/hf-alerts/resolve', {
        method: 'POST',
  ...
  async function toggleNotify(id: string, notify: boolean) {
    try {
      const res = await fetch('/api/admin/hf-alerts/notify', {
        method: 'POST',
        credentials: 'same-origin',
        headers: { 'Content-Type': 'application/json' },
  ...
  const filteredAlerts = useMemo(() => {
    return alerts.filter((a) => {
      ...
      if (searchText) {
        ...
        if (!((a.message || '').toLowerCase().includes(s) || (a.alert_type || '').toLowerCase().includes(s) || (a.email_to || '').toLowerCase().includes(s))) {
          return false
        }
      }
      return true
      ...
      <section className="mt-6">
        ...
        <div className="mt-2 flex flex-wrap items-center gap-2">
          ...
          <select value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)} className="rounded border px-2 py-1">
            ...
            {Array.from(new Set(alerts.map((a) => a.alert_type))).map((t) => (
                  ...
                  <tr key={a.id}>
                    ...
                    <td className="border px-2 py-1">
                      <div className="flex gap-2">
                        <button onClick={() => resolveAlert(a.id)} className="rounded bg-blue-600 px-2 py-1 text-sm text-white">Resolve</button>
                        <button onClick={() => toggleNotify(a.id, !a.notify)} className="rounded bg-indigo-600 px-2 py-1 text-sm text-white">{a.notify ? 'Unmark' : 'Notify'}</button>
                      </div>
                    </td>
                  </tr>
                ))}
              ...
              <tbody>
                {usage.map((u) => (
                  <tr key={u.user_id}>
                    ...
                    <td className="border px-2 py-1">
                      <button onClick={() => resetUsage(u.user_id)} className="rounded bg-yellow-600 px-2 py-1 text-sm text-white">Reset</button>
                    </td>
                  </tr>
                ))}
              </tbody>
